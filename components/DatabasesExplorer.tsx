"use client"
import { IDatabase, ISchema } from "@/util/kwil-types"
import {
  ActionIcon,
  ChevronDownIcon,
  ChevronRightIcon,
...
// https://dev.to/taiwobello/how-to-create-a-react-tree-view-component-3ch2
...
export default function DatabasesExplorer({ databases }: Props) {
  ...
  const [databaseSchemas, setDatabaseSchema] = useState<IDatabase>(databases)
  ...
  const getSchema = async (database: string) => {
    if (databaseSchemas[database]) {
      ...
      const schema: ISchema | undefined = await getDatabaseSchema(database)
      if (!schema) return

      setDatabaseSchema((prevState: IDatabase) => ({
        ...prevState,
  ...
  return (
    <div className="max-h-screen min-h-screen w-full overflow-scroll bg-slate-50/30 ">
      <ul className="flex flex-col">
        {Object.keys(databaseSchemas).map((database) => (
          // if schema exists for database in record then the tables, actions and fields are shown
          // clicking tables shows the list of tables
          // clicking actions shows the list of actions
          // clicking on specific table routes the page to the table grid view
          // clicking on specific action routes the page to the action view
          <>
            <li
              key={database}
            ...
            {databaseSchemas[database] && displayToggle[database]?.schema && (
              <div
                key={`${database}-schema`}
                className="ml-7 flex flex-1 flex-col"
              >
                <div
                  className={classNames({
                ...
                <div className="mb-1">
                  {displayToggle[database]?.tables &&
                    databaseSchemas[database]?.tables?.map((table) => (
                      <div
                        key={`${database}-${table}`}
                ...
                {displayToggle[database]?.actions &&
                  databaseSchemas[database]?.actions?.map((action) => (
                    <div key={`${database}-${action}`} className="ml-5 text-sm">
                      <Link
